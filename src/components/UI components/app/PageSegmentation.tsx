...
const PageSegmentation = ({ onSetPrevious, onSetAfter, onGoToPage, currentPage, totalItems, totalPages, sizePerPage }
    : {
        onSetPrevious: () => void,
        onSetAfter: () => void,
        onGoToPage: (key: string, page: number) => void
        currentPage: number,
        totalItems: number,
        totalPages: number,
        sizePerPage: number
    ...
    const spawnSpan = () => {
        ...
        for (let i = 0; i < totalPages; i++) {
            pageNumbers.push(
                <span
                    key={`span-${i}`}
                    className={`px-2 py-1 mx-1 cursor-pointer ${currentPage === i ? 'font-bold text-blue-600' : 'text-gray-600'
                        }`}
                    onClick={() => onGoToPage("page", i)}
                >
                    {i + 1}
                </span>
            );
